  ...
  try {
    ...
    if (isConflicted) {
      return res.status(400).send({ 
        message: `Conflict: Room ${classData.roomNumber} is already busy at ${classData.startTime} on ${classData.date}!` 
      });
    }
    ...
    res.status(500).send({ message: "Failed to assign class", error: err.message });
  }
};
...
exports.getAllAssignments = async (req, res) => {
  try {
    ...
    const result = await Schedule.find(query).sort({ date: 1, startTime: 1 });
...
exports.getTeacherTodayClasses = async (req, res) => {
  try {
    ...
    if (!teacherId) return res.status(400).send({ message: "Teacher ID is required" });
